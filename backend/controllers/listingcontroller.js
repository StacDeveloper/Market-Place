...
export const deleteUserListing = async (req, res) => {
    try {
        ...
        if (listing.isCredentialChanged) {
            const changeCredential = await inngest.send({
...
export const purchaseAccount = async (req, res) => {
    try {
        ...
        const checkoutSession = await stripeInstance.checkout.sessions.create({
            ...
            expires_at: Math.floor(Date.now() / 1000) + 30 * 60 //Session will expire in 30 min
        })
        return res.json({ success: true, paymentLink: checkoutSession.url })


    } catch (error) {
        console.log(error)
        return res.status(500).json({ success: false, message: "Api failed purchase Account", error })
    }
}