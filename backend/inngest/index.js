...
export const inngest = new Inngest({ id: "Market-Place" });
...
const syncUserDeletion = inngest.createFunction(
    ...
    async ({ event }) => {
        ...
        if (listings.length === 0 && chats.length === 0 && transaction.length === 0) {
...
const sendPurchaseEmail = inngest.createFunction(
    ...
    async ({ event }) => {
        ...
        await sendEmail({
            ...
            <div>
            ${credential.updatedCredential.map((cred) => `<p>${cred.name} : ${cred.values}</p>`).join("")}
            </div>
    ...
    async ({ event }) => {
        ...
        if (newCredential) {
            await sendEmail({
                ...
                html: `
                <div>
                <h2>Your new credentails for your deleted listing :</h2>
                title : ${listing.title}
                <br/>
                ...
                <h3>New Credentials</h3>
                <div>
            ${newCredential.updatedCredential.map((cred) => `<p>${cred.name} : ${cred.values}</p>`).join("")}
            </div>
            <h3>Old Credentials</h3>
            <div>
            ${newCredential.originalCredential.map((cred) => `<p>${cred.name} : ${cred.values}</p>`).join("")}
            </div>
            <p>If you have any questions, please contact us at <a href={"mailto:support@example.com"}>support@example.com</a></p>
                </div>
                `
